...
void declare_properties(const upb_msgdef* msgdef, zend_class_entry* ce) {
  ...
  for (upb_msg_field_begin(&it, msgdef); !upb_msg_field_done(&it);
    ...
    // Declare property in class entry for this field.
    const char* name = upb_fielddef_name(field);
    int name_len = strlen(name);
    if (upb_fielddef_label(field) == UPB_LABEL_REPEATED) {
      zend_declare_property_null(ce, name, name_len, ZEND_ACC_PUBLIC TSRMLS_CC);
    } else {
      switch (upb_fielddef_type(field)) {
        case UPB_TYPE_FLOAT:
        case UPB_TYPE_DOUBLE:
          zend_declare_property_double(ce, name, name_len, 0,
                                       ZEND_ACC_PUBLIC TSRMLS_CC);
          break;
        case UPB_TYPE_BOOL:
          zend_declare_property_bool(ce, name, name_len, 0,
                                     ZEND_ACC_PUBLIC TSRMLS_CC);
          break;
        case UPB_TYPE_STRING:
        case UPB_TYPE_BYTES:
          ...
          break;
        case UPB_TYPE_MESSAGE:
          zend_declare_property_null(ce, name, name_len,
                                     ZEND_ACC_PUBLIC TSRMLS_CC);
          break;
        case UPB_TYPE_ENUM:
        case UPB_TYPE_INT32:
        ...
        case UPB_TYPE_INT64:
        case UPB_TYPE_UINT64:
          zend_declare_property_long(ce, name, name_len, 0,
                                     ZEND_ACC_PUBLIC TSRMLS_CC);
          break;
        default:
          zend_throw_exception(zend_exception_get_default(TSRMLS_C),
                               "Unsupported field type.", 0 TSRMLS_CC);
      }
    }
  }
}
...
void layout_init(MessageLayout* layout, void* storage) {
  ...
  for (upb_msg_field_begin(&it, layout->msgdef); !upb_msg_field_done(&it);
    ...
    if (upb_fielddef_containingoneof(field)) {
      ...
      //       fieldtype_to_ruby(upb_fielddef_type(value_field)), type_class,
      //   };
      ...
      // if (type_class != Qnil) {
      //   VALUE args[2] = {
      //       fieldtype_to_ruby(upb_fielddef_type(field)), type_class,
      //   };
